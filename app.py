...
/* ---- BOTOES STREAMLIT (sub-abas e acao) ---- */
.stButton > button {
...
/* ---- OCULTAR BOTOES DE NAV (ficam atras do HTML) ---- */
.nav-hidden-btns { position: absolute; opacity: 0; pointer-events: none; height: 0; overflow: hidden; }
...
.app-sub { color: #64748b; font-size: 0.82rem; margin-top: 2px; }

/* ---- MENU PRINCIPAL: CARDS GRANDES ---- */
.nav-grid {
    display: grid;
...
.nav-card-icon { font-size: 2rem; display: block; margin-bottom: 8px; }
.nav-card-label {
    color: #94a3b8;
    font-size: 0.82rem;
    font-weight: 700;
...
.nav-card.active .nav-card-label { color: #c7d2fe; }
...
.prog-wrap {
    ...
    border-radius: 99px; height: 10px; margin: 0 0 26px; overflow: hidden;
}
.prog-fill {
...
.fb-ok {
    ...
    box-shadow: 0 0 24px rgba(52,211,153,0.25); animation: bounceIn 0.4s ease;
}
...
def gerar_pdf_membros(membros: list) -> bytes:
    ...
    for m in membros:
        ...
        card = Table([[Paragraph("CHECK-IN QR CODE — MUSICAL CCB", hs),""],
                      [ri, info],
...
<div class="app-header">
    <div>
        <div class="app-logo">🎵 CCB <span>Musical</span></div>
        <div class="app-sub">Sistema de controle de presença</div>
    </div>
</div>''', unsafe_allow_html=True)

# Cards de navegacao (HTML visual)
ativo = st.session_state.pagina
cards_html = '<div class="nav-grid">'
for key, icon, label in PAGINAS:
    cls = "nav-card active" if ativo == key else "nav-card"
    cards_html += f'<div class="{cls}"><span class="nav-card-icon">{icon}</span><span class="nav-card-label">{label}</span></div>'
cards_html += '</div>'
st.markdown(cards_html, unsafe_allow_html=True)

# Botoes reais do Streamlit (clicaveis, sobrepostos)
cols = st.columns(4)
...
st.markdown('<hr style="border:none;border-top:1px solid rgba(99,102,241,0.2);margin:0 0 24px">', unsafe_allow_html=True)
...
if pagina == "home":
    ...
    st.markdown('<div class="sec-header">📅 Reuniões Cadastradas</div>', unsafe_allow_html=True)
    hoje = agora_br().date()
    for r in reunioes:
...
# ===================================================
    ...
    <div class="banner">
        ...
        <div>
            ...
            <p class="banner-sub">Escaneie o QR Code ou digite o código</p>
        </div>
    </div>''', unsafe_allow_html=True)
    ...
    with tab_cod:
        cod_manual = st.text_input("Código (ex: CF001)", key="cod_manual").upper().strip()
...
# ===================================================
    ...
    <div class="banner">
        <span class="banner-icon">📊</span>
        <div><p class="banner-title">Relatórios</p>
        <p class="banner-sub">Frequência e presenças por reunião</p></div>
    </div>''', unsafe_allow_html=True)

    reunioes = load_reunioes()
    df_p     = load_participantes()
    if not reunioes: st.info("Nenhuma reunião."); st.stop()
    ...
    if presencas:
        ...
        for ci, h in enumerate(df_pres.columns, 1):
...
# ===================================================
    ...
    <div class="banner">
        <span class="banner-icon">👥</span>
        <div><p class="banner-title">Gerenciar Membros</p>
        <p class="banner-sub">Incluir, editar, excluir e gerar QR Code PDF</p></div>
    </div>''', unsafe_allow_html=True)
    ...
    with aba[2]:
        if df.empty: st.info("Nenhum membro.")
        else:
            opts  = df["id"] + " — " + df["nome"]
            sel_e = st.selectbox("Membro", opts, key="sel_editar")
            mid   = sel_e.split(" — ")[0]
            ...
            iv = mrow.get("instrumento") or ""
            e_instr = st.selectbox("Instrumento", INSTRUMENTOS, index=INSTRUMENTOS.index(iv) if iv in INSTRUMENTOS else 0, key="e_instr")
            e_local = st.text_input("Localidade", value=mrow["localidade"], key="e_local").upper().strip()
            if st.button("💾 Salvar", type="primary", key="btn_editar"):
    ...
    with aba[3]:
        if df.empty: st.info("Nenhum membro.")
            ...
            sel_d  = st.selectbox("Membro para excluir", opts_d, key="sel_excluir")
            mid_d  = sel_d.split(" — ")[0]
            st.warning(f"⚠️ Excluir **{sel_d}**? Remove também as presenças vinculadas.")
    ...
    with aba[4]:
        if not QR_OK or not RL_OK:
            st.error("Instale: qrcode e reportlab")
        else:
