...
.stTextInput input:focus {
    ...
    box-shadow:0 0px 0 1px rgba(99,102,241,0.2) !important;
}
...
def gerar_pdf_membros(membros: list) -> bytes:
    ...
    name_style   = ParagraphStyle("name",   fontName="Helvetica-Bold", fontSize=11,
    ...
    code_style   = ParagraphStyle("code",   fontName="Helvetica-Bold", fontSize=13,
    ...
    for m in membros:
        ...
        buf_qr.seek(0)
        rl_img = RLImage(buf_qr, width=3.5*cm, height=3.5*cm)

        instr = m.get("instrumento") or ""
        local = m.get("localidade") or ""
        info = [
            Paragraph(m["nome"], name_style),
            Spacer(1, 4),
            Paragraph(f'<font color="#555555">Cargo: </font><b>{m["cargo"]}</b>', label_style),
            Paragraph(f'<font color="#555555">Local: </font><b>{local}</b>', label_style),
            Paragraph(f'<font color="#555555">Instr.: </font><b>{instr}</b>', label_style),
        ]
        card = Table([
            [Paragraph("CHECK-IN QR CODE — MUSICAL CCB", header_style), ""],
            [rl_img, info],
...
# -------------------------------------------------------
with st.sidebar:
    st.markdown("### 🎵 CCB Musical")
    st.markdown("---")
    pagina = st.radio(
        "Navegação",
        ["🏠 Home", "📷 Check-in", "📊 Relatórios", "👥 Gerenciar Membros"],
...
if pagina == "🏠 Home":
    ...
    <div class="metric-row">
        <div class="metric-card mc-blue">
            <p class="metric-value">{total_m}</p>
            <p class="metric-label">Membros</p>
        </div>
        <div class="metric-card mc-purple">
            <p class="metric-value">{total_r}</p>
            <p class="metric-label">Reuniões</p>
        </div>
        <div class="metric-card mc-green">
            <p class="metric-value">{total_pr}</p>
            <p class="metric-label">Presenças</p>
        </div>
        <div class="metric-card mc-red">
            <p class="metric-value">{perc}%</p>
            <p class="metric-label">Freq. Geral</p>
        </div>
    </div>
    ''', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════
    ...
    reunioes = load_reunioes()
    if not reunioes:
        st.warning("Nenhuma reunião cadastrada.")
        st.stop()
    ...
    with tab_cam:
        ...
        if img_file:
            ...
            if decoded:
                ...
                if status == "ok":
                    st.markdown(f'<div class="fb-ok"><p class="fb-title">✅ Check-in realizado!</p><p class="fb-nome">{m["nome"]}</p><p style="color:#6ee7b7">{m["cargo"]} — {m["localidade"]}</p></div>', unsafe_allow_html=True)
                elif status == "duplicate":
                    st.markdown(f'<div class="fb-warn"><p class="fb-title">⚠️ Já registrado: {m["nome"]}</p></div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="fb-erro"><p class="fb-title">❌ Código não encontrado</p></div>', unsafe_allow_html=True)
            else:
    ...
    with tab_cod:
        ...
        if st.button("✅ Registrar", type="primary", key="btn_manual"):
            if cod_manual:
                ...
                if status == "ok":
                    st.markdown(f'<div class="fb-ok"><p class="fb-title">✅ Check-in realizado!</p><p class="fb-nome">{m["nome"]}</p></div>', unsafe_allow_html=True)
                elif status == "duplicate":
                    st.markdown(f'<div class="fb-warn"><p class="fb-title">⚠️ Já registrado: {m["nome"]}</p></div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="fb-erro"><p class="fb-title">❌ Código não encontrado</p></div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════
# PÁGINA: GERENCIAR MEMBROS
# ═══════════════════════════════════════════════════════
elif pagina == "👥 Gerenciar Membros":
    st.markdown('''
    <div class="banner">
        <span class="banner-icon">👥</span>
        <div>
    ...
    INSTRUMENTOS = [
        "CLARINETE", "FLAUTA", "ÓRGÃO", "SAXOFONE ALTO",
        "SAXOFONE TENOR", "TROMBONE", "TROMPETE", "TUBA",
        "VIOLINO", "VIOLONCELO"
    ]
    CARGOS = [
        "ENCARREGADO REGIONAL", "ENCARREGADO LOCAL",
    ...
    with aba[0]:
        ...
        if filtro_cargo != "Todos":
            df_view = df_view[df_view["cargo"] == filtro_cargo]
        if filtro_instr != "Todos":
            df_view = df_view[df_view["instrumento"] == filtro_instr]
            ...
            if not n_id or not n_nome or not n_local:
                ...
                if chk.data:
                    ...
                    supabase.table("participantes").insert({
                        "id": n_id, "nome": n_nome, "cargo": n_cargo,
        ...
        if df.empty:
            ...
            if st.button("💾 Salvar Alterações", type="primary", key="btn_editar"):
                supabase.table("participantes").update({
                    "nome": e_nome, "cargo": e_cargo,
                    "instrumento": e_instr, "localidade": e_local
                }).eq("id", mid).execute()
